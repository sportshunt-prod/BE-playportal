...
Tokens are returned in JSON from the authentication endpoints and should be sent by clients using the Authorization header on subsequent requests:

Authorization: Bearer <token>

The server validates the Bearer token from the Authorization header. If you need cookie-based storage (browser-only), implement it deliberately and pair it with CSRF protections; the current server code expects the Authorization header only.
...
class AuthTestCase(TestCase):
    ...
    # Token is returned in JSON under 'token'
    data = response.json()
...
```javascript
// Registration (example: client stores token in memory and sends as Authorization header)
async function register(email, username, password, passwordConfirm) {
  ...
  if (response.ok) {
    ...
    // store token in memory or a secure client-managed store
...
// Logout
function logout() {
  // Client should drop the stored token; server-side logout is client-side only for bearer tokens
}
```
...
// Use the instance: include Authorization header on requests
// Example after login: api.defaults.headers.common['Authorization'] = `Bearer ${token}`
api
